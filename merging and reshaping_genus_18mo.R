...
setwd("C:/Users/scvogel/Dropbox/BASELab/DMMProject/Data/Microbiomedata/18 month/sequencing/NON-HOST/HUMANN/All/genus")
setwd("/Users/sarahvogel/Library/CloudStorage/Dropbox/BASELab/DMMProject/Data/Microbiomedata/18 month/sequencing/NON-HOST/HUMANN/All/genus")


###Before this step, manually inspect the genus files and delete any that are empty
library(dplyr)
library(readr)
library(tidyr)
library(reshape2)
...
#not merging correctly, try again later
...
str(merged_data$relative_abundance) #check to make sure this is read as numeric and recode with the next line if needed
...
colnames(transposed_data) <- gsub("^(Nicolas_\\d+).*", "\\1", colnames(transposed_data)) #fix sequencing IDs in the column name, just retain Nicolas_## part

write.csv(transposed_data, "genus_abundances_18mo.csv")
write_tsv(transposed_data, "genus_abundances_18mo.tsv")
